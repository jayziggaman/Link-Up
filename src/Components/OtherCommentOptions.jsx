...
const OtherCommentOptions = ({ func, postId }) => {
  const { userAuth, allPosts, user, processingType, setProcessing, users, showOtherComOptionsDIv, setShowOtherComOptionsDiv } = useContext(appContext)
  const navigate = useNavigate()

  useEffect(() => {
    const optionsDiv = document.querySelector('.other-comment-options-div')
    if (showOtherComOptionsDIv) {
  ...
  }, [showOtherComOptionsDIv, allPosts])
  ...
  return (
    <div role={'button'} onClick={() => setShowOtherComOptionsDiv(false)} className='other-comment-options-div'>
      <div>
        {func?.map((x, index) => {
          return <button key={index} className='option' onClick={e => {
            handleFunc(e, x.id)
            setShowOtherComOptionsDiv(false)
          }}>
