...
const OtherReplyOptions = ({func, postId, CommentId}) => {
  const { userAuth, user, processingType, setProcessing, users, showOtherRepOptionsDIv, setShowOtherRepOptionsDiv } = useContext(appContext)
  const navigate = useNavigate()
  
  useEffect(() => {
    const optionsDiv = document.querySelector('.other-reply-options-div')
    if (showOtherRepOptionsDIv) {
  ...
  return (
    <div role={'button'} onClick={() => setShowOtherRepOptionsDiv(false)} className='other-reply-options-div'>
      <div>
        {func.map((x, index) => {
          return <button key={index} className='option' onClick={e => {
            handleFunc(e, x.id)
            setShowOtherRepOptionsDiv(false)
          }}>
...
export default OtherReplyOptions