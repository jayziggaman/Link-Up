...
const ForwardedStory = () => {
  const {
    users, user, setShowStoryForm, storyType, deleteStory, followStory, followingStories, setFollowingStories, userAuth, gError, year, mins, period, day, month, hours, sssMenu, setSssMenu, userDirectMessages, currentStory
  ...
  useEffect(() => {
    setStoryCreator(users.find(user => user.id === followStory.current))
    const userRef = collection(usersRef, followStory.current, 'stories')
    ...
    onSnapshot(userQuery, snap => {
      ...
      setUserStories([...arr])
      setCurrStory(arr.find(story => story.id === storyId))
      const ind = arr.findIndex(story => story.id === storyId)
      setIndex(prev => (prev - prev) + ind)
    })
  }, [])
  ...
  useEffect(() => {
    if (indexRef.current) {
      ...
      if (location.pathname !== `/f/stories/${storyCreator.username}/${userStories[index]?.id}`) {
        navigate(`/f/stories/${storyCreator.username}/${userStories[index]?.id}`)
      } else return
    }
  }, [index])

  const storyIdRef = useRef(true)
  useEffect(() => {
    if (storyIdRef.current) {
      storyIdRef.current = false
    } else {
      setCurrStory(userStories.find(story => story.id === storyId))
      const ind = userStories.findIndex(story => story.id === storyId)
      setIndex(prev => (prev - prev) + ind)
    }
  }, [location.pathname])
  ...
  useEffect(() => {
    if (location.state) {
      setReturnUrl(location.state.url)
      setreturnUser(location.state.user)
    }
  }, [location.pathname])
      ...
      <div className='story-layout' onClick={() => {
        ...
        <div className='story-options'>
          <div role={'button'} onClick={() => {
            navigate(returnUrl, {state: returnUser}) 
          ...
          <div className="story-details">
            ...
            <div className="story-abs" id="story-abs">
              ...
              <div className='story-amt'>
                {userStories.map((story, index) => <div key={index}></div>)}
              </div>
